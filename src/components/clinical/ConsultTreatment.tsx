    ...
    return (
        <div className="flex flex-col h-full animate-in fade-in zoom-in-95 duration-500 relative">
            <header className="mb-8 flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-black text-slate-900 px-1 tracking-tight uppercase">Treatment Plan</h2>
                    <div className="flex items-center gap-2 px-1 mt-1">
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Active Care Protocol</p>
                    </div>
                </div>
                ...
                <div className="bg-slate-50 p-1 rounded-xl border border-slate-100 flex gap-1">
                    <button
                        ...
                        className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === 'new' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}
                    >
                        NEW
                    </button>
                    <button
                        onClick={() => setActiveTab('history')}
                        className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === 'history' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}
                    >
                        HISTORY
                    </button>
                </div>
                        ...
                        >
                            ...
                            <div className="space-y-3">
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-slate-50 border-2 border-slate-100 rounded-[2rem] p-6 text-sm font-bold text-slate-700 outline-none focus:bg-white focus:border-indigo-500/30 transition-all resize-none italic"
                                />
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-slate-50 border-2 border-slate-100 rounded-[2rem] p-6 text-sm font-bold text-slate-700 outline-none focus:bg-white focus:border-indigo-500/30 transition-all resize-none"
                                />
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-white border-2 border-slate-100 rounded-[2rem] p-6 text-sm font-bold text-slate-700 outline-none focus:border-indigo-600/30 transition-all resize-none shadow-sm"
                                />
                            </div>

                            {/* Quick Select Chips */}
                            <div className="space-y-4">
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Quick Add Recommendations</p>
                                <div className="flex flex-wrap gap-2">
                                    {recommendations.map((rec, i) => (
                                        <button
                                            key={i}
                                            onClick={() => addRecommendation(rec)}
                                            className="px-4 py-2 bg-slate-50 hover:bg-indigo-50 border border-slate-100 hover:border-indigo-200 text-slate-500 hover:text-indigo-600 rounded-xl text-[9px] font-black uppercase tracking-tight transition-all"
                                        >
                                            + {rec}
                                        </button>
                                    ))}
                        ...
                        >
                            {loading ? (
                                ...
                                <div className="text-center py-20 bg-slate-50 border-2 border-dashed border-slate-100 rounded-[3rem]">
                                    ...
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">No Historical Plans Found</p>
                                </div>
                            ) : (
                                history.map((record, idx) => (
                                    <div key={record.id || idx} className="card-premium p-8 bg-white border-slate-100 space-y-6 hover:border-indigo-100 transition-all group">
                                        <div className="flex items-center justify-between border-b border-slate-50 pb-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Treatment Snapshot</p>
                                                </div>
                                            </div>
                                        ...
                                        <div className="space-y-4">
                                            {record.plan && (
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="text-sm font-bold text-slate-700 italic leading-relaxed">"{record.plan}"</p>
                                                </div>
                                            )}
                                            {record.medications && (
                                                <div className="space-y-1">
                                                    <p className="text-[8px] font-black text-rose-400 uppercase tracking-widest">Medications</p>
                                                    <p className="text-sm font-bold text-slate-700 leading-relaxed capitalize">{record.medications}</p>
                                                </div>
                                            )}
                                            {record.next_steps && (
                                                <div className="space-y-1">
                                                    <p className="text-[8px] font-black text-emerald-400 uppercase tracking-widest">Next Steps</p>
                                                    <p className="text-sm font-bold text-slate-700 leading-relaxed whitespace-pre-wrap">{record.next_steps}</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
