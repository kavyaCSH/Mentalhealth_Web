            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="w-16 h-16 bg-card rounded-3xl flex items-center justify-center text-indigo-600 shadow-2xl shadow-indigo-100 border border-indigo-50 glow-primary relative overflow-hidden group">
                        ...
                        <div>
                            ...
                            <h1 className="text-5xl font-black text-main tracking-tighter">Facility Health</h1>
                        </div>
                    </div>
                </div>
                <div className="flex gap-4">
                    <Button variant="outline" size="lg" className="px-8 border-border-card" leftIcon={<Search size={18} />}>
                        Diagnostic Search
                    </Button>
                ...
                {stats.map((stat, i) => (
                    ...
                    >
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <div className="text-[10px] font-black text-muted opacity-80 uppercase tracking-widest px-2.5 py-1 bg-page rounded-lg">
                                {stat.trend}
                            </div>
                        </div>
                        <div>
                            <h3 className="text-4xl font-black text-main tracking-tight">{stat.value}</h3>
                            <p className="text-xs font-black text-muted opacity-80 uppercase tracking-widest mt-2">{stat.label}</p>
                        </div>
            ...
            <div className="grid gap-8 lg:grid-cols-3">
                ...
                <section className="lg:col-span-2 space-y-8">
                    <div className="flex items-center justify-between px-2">
                        <div className="flex items-center gap-4">
                            ...
                            <h2 className="text-2xl font-black text-main tracking-tight">Infrastructure Matrix</h2>
                        </div>
                        <span className="text-[10px] font-bold text-muted opacity-80 uppercase tracking-[0.2em]">Scale: Real-time</span>
                    </div>
                        ...
                        {departmentStatus.map((dept, i) => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-10">
                                    <div>
                                        <h3 className="text-xl font-black text-main mb-1">{dept.name}</h3>
                                    ...
                                    <div className="w-10 h-10 bg-page flex items-center justify-center rounded-xl text-muted opacity-80 group-hover:text-indigo-600 transition-colors">
                                        <ChevronRight size={20} />
                                    </div>
                                </div>
                                <div className="space-y-4">
                                    <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-muted opacity-80">
                                        <span>Load Distribution</span>
                                        <span className="text-main">{dept.load}%</span>
                                    </div>
                    ...
                    <div className="p-12 text-center space-y-6 bg-card/50 rounded-[3rem] border border-dashed border-border-card">
                        <div className="flex justify-center gap-4">
                            {[Server, Database, Cloud].map((Icon, i) => (
                                <div key={i} className="w-12 h-12 bg-card rounded-2xl flex items-center justify-center text-muted opacity-40 border border-border-card shadow-sm">
                                    <Icon size={24} />
                                </div>
                            ))}
                        </div>
                        <div className="space-y-2">
                            <h3 className="text-lg font-bold text-main italic tracking-tight">Syncing Operational Logs...</h3>
                            <p className="text-sm text-muted max-w-md mx-auto leading-relaxed">The high-frequency diagnostic analyzer is processing telemetry data from <b>14 departments</b>. Advanced metrics will appear shortly.</p>
                        </div>
                        <Button variant="outline" className="rounded-2xl border-border-card font-black uppercase tracking-widest text-[10px] px-8 py-3">
                ...
                <section className="space-y-8">
                    <div className="flex items-center gap-4 px-2">
                        ...
                        <h2 className="text-2xl font-black text-main tracking-tight">System Alerts</h2>
                    </div>
                    ...
                    <div className="space-y-4">
                        {[
                            ...
                            <motion.div
                                ...
                                className="group p-6 bg-card border border-border-card rounded-3xl hover:border-indigo-100 hover:shadow-2xl hover:shadow-indigo-100/50 transition-all cursor-pointer relative overflow-hidden"
                            >
                                <div className="flex gap-5 relative z-10">
                                    <div className="shrink-0 w-12 h-12 rounded-2xl bg-page flex items-center justify-center border border-border-card group-hover:bg-slate-900 group-hover:text-white transition-all duration-300">
                                        <AlertCircle size={24} />
                                    </div>
                                    <div className="space-y-2">
                                        <div className="flex items-center gap-3">
                                            <span className={`w-1.5 h-1.5 rounded-full ${alert.pulse} animate-pulse`} />
                                            <span className="text-[8px] font-black text-muted opacity-80 tracking-[0.2em]">{alert.tag}</span>
                                        </div>
                                        <p className="text-sm font-black text-main leading-tight">{alert.title}</p>
                                        <p className="text-[10px] text-muted font-bold leading-relaxed">{alert.desc}</p>
                                    </div>
                                </div>
                    ...
                    <div className="p-8 bg-indigo-900 rounded-[2.5rem] text-white shadow-2xl shadow-indigo-200/50 relative overflow-hidden group">
                        ...
                        <div className="relative z-10 space-y-6">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-card/10 backdrop-blur-xl rounded-2xl flex items-center justify-center border border-white/20">
                                    <Activity size={24} />
                                </div>
                                <div>
