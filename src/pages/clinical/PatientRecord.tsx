...
const PatientRecord = () => {
        ...
        const fetchPatientData = async () => {
            ...
            try {
                ...
                if (id) {
                    ...
                    setStats(s);
                    if (p) {
                        const nRes = await UserService.getClinicalNotes(p.userId || p._id || id).catch(() => []);
                        setNotes(nRes);
                    }
                }
            } finally {
    ...
    const handleAddNote = async () => {
        ...
        try {
            const added = await UserService.addClinicalNote(patient.userId || patient._id || id!, { content: newNote });
    ...
    if (!patient) return <div className="p-8 text-center text-muted">Patient not found</div>;

    return (
        <div className="p-8 max-w-7xl space-y-8 animate-fade-in pb-20">
            <header className="flex flex-col md:flex-row md:items-start justify-between gap-6 pb-6 border-b border-border-card">
                <div className="flex items-start gap-6">
                    <button onClick={() => navigate('/patients')} className="p-2.5 bg-page hover:bg-page rounded-xl text-muted transition-colors mt-2">
                        <ChevronLeft size={20} />
                    </button>
                    <div>
                        ...
                        <p className="text-muted font-bold uppercase tracking-widest text-xs flex items-center gap-2">
                            ID: <span className="font-mono bg-page px-2 py-0.5 rounded text-muted">{patient.id}</span>
                        </p>
                    </div>
                </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                    ...
                    <div className="card-premium p-6">
                        <h3 className="text-xs font-black text-muted opacity-80 uppercase tracking-widest mb-6">Patient Demographics</h3>

                        <div className="space-y-5">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-xl bg-page border border-border-card flex items-center justify-center text-muted opacity-80 shrink-0">
                                    <Phone size={16} />
                                </div>
                                <div>
                                    <p className="text-[10px] font-black text-muted opacity-80 uppercase tracking-widest">Phone Number</p>
                                    <p className="text-sm font-bold text-main">{patient.phone}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    <p className="text-[10px] font-black text-muted opacity-80 uppercase tracking-widest">Email Address</p>
                                    <p className="text-sm font-bold text-main truncate">{patient.email}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-muted opacity-80 uppercase tracking-widest">Date of Birth</p>
                                    <p className="text-sm font-bold text-main">{patient.dob ? new Date(patient.dob).toLocaleDateString() : 'N/A'}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-muted opacity-80 uppercase tracking-widest mb-1">Gender</p>
                                    <p className="text-sm font-bold text-main">{patient.gender || 'N/A'}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {!isFocused && (
                        <div className="card-premium p-6 flex flex-col h-[500px]">
                            ...
                            <div className="flex-1 overflow-y-auto pr-2 space-y-4 no-scrollbar mb-4">
                                {notes.length > 0 ? notes.map((note) => (
                                    <motion.div
                                        ...
                                        key={note.id}
                                        className="p-4 bg-page rounded-2xl border border-border-card"
                                    >
                                        <p className="text-main font-medium leading-relaxed mb-3 text-sm">{note.content}</p>
                                        <div className="flex items-center justify-between text-xs font-bold text-muted opacity-80">
                                    ...
                                    <div className="h-full flex flex-col items-center justify-center text-muted opacity-80">
                            ...
                            <div className="shrink-0 flex gap-3 pt-4 border-t border-border-card">
                                <InputField
                    ...
                    {!isFocused && stats?.moodAnalytics?.distribution && (
                        <div
                            ...
                            className="card-premium p-6 border-border-card hover:border-indigo-100 transition-all cursor-pointer group"
                        >
                            <div className="flex items-center justify-between mb-8">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[10px] font-black text-muted opacity-80 uppercase tracking-widest italic">Emotional Distribution Matrix</p>
                                </div>
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                                    ...
                                    className="p-2 bg-page text-muted opacity-80 hover:text-indigo-600 rounded-xl transition-all hover:scale-110 active:scale-95"
                                    ...
                                    return (
                                        <div key={mood} className="space-y-2">
                                            <div className="flex justify-between items-end">
                                                ...
                                                <span className="text-xs font-black text-main italic">{Math.round(percentage)}%</span>
                                            </div>
                                            <div className="h-1.5 bg-page rounded-full overflow-hidden border border-border-card/50">
                                                <motion.div
                                                    initial={{ width: 0 }}
                            ...
                            <div className="mt-8 pt-4 border-t border-border-card flex items-center justify-between text-[9px] font-black text-muted opacity-80 uppercase tracking-widest">
                    ...
                    {!isFocused && (
                        ...
                        >
                            <div className="absolute top-0 right-0 w-32 h-32 bg-card/10 blur-3xl rounded-full translate-x-10 -translate-y-10 group-hover:scale-150 transition-transform duration-700" />

                            <div className="relative z-10 flex flex-col gap-6">
                                <div className="w-12 h-12 bg-card/20 backdrop-blur-md rounded-2xl flex items-center justify-center border border-white/20">
                    ...
                    {!isFocused && history.length > 0 && (
                        <div className="card-premium p-6 border-indigo-100 bg-card/50 backdrop-blur-sm relative overflow-hidden group">
                            ...
                            <div className="flex items-center gap-8 py-4 px-6 bg-card/50 rounded-3xl border border-white/50">
                                <div className="text-center">
                                    ...
                                    <span className="text-[9px] font-black text-muted opacity-80 uppercase tracking-widest leading-none">Mental Pulse</span>
                                </div>

                                <div className="w-px h-12 bg-border-card" />

                                <div className="flex-1">
                                    <h4 className="text-lg font-black text-main leading-tight">
                                        {history[0].interpretation}
                                    </h4>
                                    <p className="text-xs font-bold text-muted opacity-80 uppercase tracking-widest mt-1">
                    ...
                    <div className="card-premium p-6">
                        ...
                        <div className="space-y-3">
                            {history.length > 0 ? history.map((record) => (
                                <div key={record.id} className="p-4 rounded-xl border border-border-card hover:border-indigo-100 hover:bg-page transition-all flex items-center justify-between group">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="font-bold text-main lowercase first-letter:uppercase">{record.category?.replace(/_/g, ' ')}</p>
                                ...
                                <p className="text-sm font-medium text-muted text-center py-4">No assessments completed.</p>
                            )}
                        </div>
                    </div>
                </div>
