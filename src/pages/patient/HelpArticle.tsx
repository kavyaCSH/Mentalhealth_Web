...
const HelpArticle = () => {
    ...
    const renderContent = (content: string) => {
        ...
        return lines.map((line, index) => {
            if (line.startsWith('###')) {
                return (
                    <h3 key={index} className="text-2xl font-black text-main mt-12 mb-6 tracking-tight">
                        {line.replace('###', '').trim()}
                    </h3>
                );
            }
                ...
                return (
                    <div key={index} className="flex gap-4 mb-6 group">
                        ...
                        <p className="text-[17px] text-main opacity-90 leading-relaxed font-medium">
                            {parts.map((part, i) => (
                                <span key={i} className={i % 2 === 1 ? 'font-black text-main' : ''}>
                                    {part}
                                </span>
                            ))}
                        </p>
            ...
            if (line.trim().length > 0) {
                ...
                return (
                    <p key={index} className="text-[17px] text-muted mb-8 leading-relaxed font-medium">
                        {parts.map((part, i) => (
                            <span key={i} className={i % 2 === 1 ? 'font-black text-main' : 'opacity-90'}>
                                {part}
                            </span>
                        ))}
                    </p>
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-page">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-muted opacity-80 font-black uppercase tracking-widest text-[10px]">Fetching Wisdom...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-page">
            {/* Minimal Header */}
            <div className="sticky top-0 bg-page/80 backdrop-blur-xl border-b border-border-card z-30">
                <div className="max-w-3xl mx-auto px-6 h-20 flex items-center justify-between">
                    <button 
                        onClick={() => navigate(-1)}
                        className="w-10 h-10 rounded-xl bg-card flex items-center justify-center text-muted hover:text-indigo-600 hover:bg-indigo-50 transition-all"
                    >
                        <ChevronLeft size={24} />
                    </button>
                    <div className="flex-1 px-6 truncate">
                        <span className="text-xs font-black uppercase tracking-widest text-muted opacity-80 mr-2">Reading</span>
                        <span className="text-sm font-black text-main truncate">{title}</span>
                    </div>
                </div>
            </div>
            ...
            <article className="max-w-3xl mx-auto px-6 py-16">
                ...
                {/* Feedback Section */}
                <section className="mt-24 pt-12 border-t border-border-card">
