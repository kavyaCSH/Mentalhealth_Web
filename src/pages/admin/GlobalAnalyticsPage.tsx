    ...
    return (
        <div className="p-8 space-y-10 animate-fade-in max-w-7xl pb-32">
            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-4xl font-black text-main tracking-tight text-main">Global Analytics</h1>
            ...
            <div className="grid gap-6 md:grid-cols-4">
                <div className="card-premium p-6 flex flex-col justify-between border-border-card/50 bg-gradient-to-br from-white to-slate-50">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-muted opacity-80 uppercase tracking-widest mb-1">Total Consults</p>
                        <p className="text-3xl font-black text-main tracking-tight">{loading ? '—' : totalConsults}</p>
                    </div>
                </div>
                ...
                <div className="card-premium p-6 flex flex-col justify-between border-border-card/50 bg-gradient-to-br from-white to-slate-50">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-muted opacity-80 uppercase tracking-widest mb-1">Avg Global Risk Score</p>
                        <p className="text-3xl font-black text-main tracking-tight">{loading ? '—' : averageRisk}</p>
                    </div>
                </div>
                ...
                <div className="card-premium p-6 flex flex-col justify-between border-border-card/50 bg-gradient-to-br from-white to-slate-50">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-muted opacity-80 uppercase tracking-widest mb-1">High/Critical Flags</p>
                ...
                <div className="card-premium p-6 flex flex-col justify-between border-border-card/50 bg-gradient-to-br from-white to-slate-50">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-muted opacity-80 uppercase tracking-widest mb-1">Active Regions</p>
                        <p className="text-3xl font-black text-main tracking-tight">
            ...
            <div className="flex items-center gap-2 border-b border-border-card pb-2">
                <button
                    ...
                    className={`flex items-center gap-2 px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                        activeTab === 'dashboard' ? 'bg-slate-900 text-white' : 'text-muted opacity-80 hover:bg-page hover:text-main'
                    }`}
                >
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                        activeTab === 'heatmap' ? 'bg-slate-900 text-white' : 'text-muted opacity-80 hover:bg-page hover:text-main'
                    }`}
                >
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'dashboard' && analytics && (
                    ...
                    >
                        ...
                        <div className="grid lg:grid-cols-3 gap-8">
                            {/* Risk Distribution */}
                            <div className="lg:col-span-1 card-premium p-8 border-border-card/50 flex flex-col">
                                <h3 className="text-[11px] font-black text-muted opacity-80 uppercase tracking-widest mb-8 flex items-center gap-2">
                                    ...
                                    {analytics.risk_distribution.map(risk => (
                                        <div key={risk._id}>
                                            <div className="flex justify-between items-end mb-2">
                                                ...
                                                <span className="text-xl font-black text-main">{risk.count}</span>
                                            </div>
                                            <div className="w-full bg-page rounded-full h-3 border border-border-card overflow-hidden">
                                                <motion.div
                                                    initial={{ width: 0 }}
                            ...
                            {/* Regional Breakdown */}
                            <div className="lg:col-span-2 card-premium border-border-card/50 overflow-hidden">
                                <div className="p-8 border-b border-border-card bg-card/50">
                                    <h3 className="text-[11px] font-black text-muted opacity-80 uppercase tracking-widest flex items-center gap-2">
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="bg-card border-b border-border-card">
                                                <th className="px-8 py-5 text-[10px] font-black text-muted opacity-80 uppercase tracking-widest">Region</th>
                                                <th className="px-8 py-5 text-[10px] font-black text-muted opacity-80 uppercase tracking-widest">Consults</th>
                                                <th className="px-8 py-5 text-[10px] font-black text-muted opacity-80 uppercase tracking-widest">Avg Risk</th>
                                                <th className="px-8 py-5 text-[10px] font-black text-muted opacity-80 uppercase tracking-widest">Dominant Symptoms</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-100">
                                            {analytics.by_city.map((city, idx) => (
                                                ...
                                                >
                                                    <td className="px-8 py-6 font-bold text-main">{city._id}</td>
                                                    <td className="px-8 py-6 text-sm font-bold text-muted">{city.total_consults}</td>
                                                    ...
                                                    <td className="px-8 py-6">
                                                        <div className="flex flex-wrap gap-2">
                                                            ...
                                                            {(Array.isArray(city.symptom_summary) ? city.symptom_summary.flat() : []).length > 3 && (
                                                                <span className="px-2 py-1 bg-page text-muted opacity-80 rounded text-[9px] font-bold">
                                                                    +{(Array.isArray(city.symptom_summary) ? city.symptom_summary.flat() : []).length - 3}
                                                                </span>
                                                            )}
                                                        </div>
                                            ...
                                            {analytics.by_city.length === 0 && (
                                                <tr>
                                                    <td colSpan={4} className="px-8 py-10 text-center text-sm font-medium text-muted opacity-80">
                ...
                {activeTab === 'heatmap' && (
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -10 }}
                        className="card-premium border-border-card/50 p-1 space-y-0 overflow-hidden relative"
                    >
                        {/* MapContainer replacing placeholder */}
                        <div className="relative h-[400px] bg-page rounded-t-[30px] overflow-hidden border-b border-border-card/50">
                            <MapContainer 
                                center={[20, 0]} 
                                zoom={2} 
                                    ...
                                    >
                                        <Popup className="font-sans">
                                            <div className="min-w-[150px]">
                                                <p className="font-bold text-main mb-1">{node.city}</p>
                                                <p className="text-xs text-muted mb-2">{node.primary_diagnosis}</p>
                        ...
                        <div className="p-8 bg-card max-h-[500px] overflow-y-auto">
                            <h3 className="text-[11px] font-black text-muted opacity-80 uppercase tracking-widest mb-6 flex items-center gap-2">
                                <Brain size={16} /> Diagnostic Clusters (Raw Telemetry)
                            </h3>
                            
                            <div className="grid gap-4 md:grid-cols-2">
                                {heatMap.map((node, i) => (
                                    <div key={node._id || i} className="p-5 rounded-2xl border border-border-card bg-page flex flex-col justify-between hover:border-indigo-200 transition-colors">
                                        <div className="flex items-start justify-between mb-4">
                                            <div>
                                                <p className="font-bold text-main">{node.city}</p>
                                                <p className="text-[10px] font-mono text-muted mt-1">lat: {node.coordinates.lat.toFixed(4)}, lng: {node.coordinates.lng.toFixed(4)}</p>
                                            </div>
                                        ...
                                        <div className="pt-4 border-t border-border-card/50">
                                            <p className="text-[9px] font-black text-muted opacity-80 uppercase tracking-widest mb-1">Primary Diagnosis</p>
                                            <p className="text-sm font-bold text-main">{node.primary_diagnosis}</p>
                                        </div>
                                    </div>
                                ))}
                                ...
                                {heatMap.length === 0 && (
                                    <div className="md:col-span-2 text-center py-10">
                                        <p className="text-sm font-medium text-muted opacity-80">No geo-tagged diagnostic data available.</p>
                                    </div>
                                )}
                            </div>
                        </div>
